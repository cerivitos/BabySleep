...
(function () {
    ...
    */
    ...
    function create_if_block_1(ctx) {
    	...
    	return {
    		c: function create() {
    			button = element("button");
    			t = text("Sign In");
    			button.className = button_class_value = "w-1/2 p-4 rounded " + (ctx.isSignedIn ? 'bg-secondaryColor opacity-50 cursor-not-allowed' : 'bg-accentColor') + "\r\n    text-center text-backgroundColor";
    			add_location(button, file$2, 115, 2, 2652);
    			dispose = listen(button, "click", ctx.click_handler);
    		},
    		...
    		p: function update(changed, ctx) {
    			if ((changed.isSignedIn) && button_class_value !== (button_class_value = "w-1/2 p-4 rounded " + (ctx.isSignedIn ? 'bg-secondaryColor opacity-50 cursor-not-allowed' : 'bg-accentColor') + "\r\n    text-center text-backgroundColor")) {
    ...
    function create_if_block(ctx) {
    	...
    	return {
    		c: function create() {
    			...
    			t = text("Sign out");
    			button.className = button_class_value = "w-1/2 p-4 rounded " + (!ctx.isSignedIn ? 'bg-secondaryColor opacity-50 cursor-not-allowed' : 'bg-accentColor') + "\r\n    text-center text-backgroundColor";
    			add_location(button, file$2, 123, 2, 2899);
    ...
    function create_fragment$2(ctx) {
    	var meta, meta_content_value, t0, t1, if_block1_anchor;

    	var if_block0 = (!ctx.isSignedIn) && create_if_block_1(ctx);

    	var if_block1 = (ctx.isSignedIn) && create_if_block(ctx);
    ...
    function instance$2($$self, $$props, $$invalidate) {
      ...
      function signIn() {
        gapi.auth2
          ...
          .signIn()
          .then(response => {
            response.El.length > 0 ? (isSignedIn = true) : (isSignedIn = false); $$invalidate('isSignedIn', isSignedIn);
    ...
    function create_fragment$3(ctx) {
    	...
    	return {
    		c: function create() {
    			...
    			add_location(main, file$3, 14, 0, 488);
    		},
